...
events.on('catalog:changed', () => {
  ...
  const cards = products.map((product) => {
    const node = catalogTemplate.content.firstElementChild!.cloneNode(true) as HTMLElement;
...
events.on('preview:changed', () => {
  ...
  const node = previewTemplate.content.firstElementChild!.cloneNode(true) as HTMLElement;
...
events.on('basket:changed', () => {
  ...
  const renderedItems = items.map((product, index) => {
    const node = basketItemTemplate.content.firstElementChild!.cloneNode(true) as HTMLElement;
...
events.on('order:submit', () => {
  const formNode = contactsTemplate.content.firstElementChild!.cloneNode(true) as HTMLFormElement;
  contactsFormView = new ContactsForm(formNode, events);

  modal.render({ content: contactsFormView.render({ valid: false, errors: '' }) });
  modal.open();
});
